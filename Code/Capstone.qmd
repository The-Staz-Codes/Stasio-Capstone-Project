...
The column names were already updated from the original dataset and documented in the metadata file. Thus, there is no need to adjust any column names here. 
Commentary on the non-factor data types present:
-secret_name and dmt_type_overall are categorical data 
-age_category, edss_category, covid19_outcome, and comorbidities_other are discrete count data that represent categories, which are detailed in the metadata file 
-year_onset and end_date are continuous, interval data 
